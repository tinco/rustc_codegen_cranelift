...
fn module_codegen(tcx: TyCtxt<'_>, cgu_name: rustc_span::Symbol) -> CompiledModule {
    ...
    let (mut cx, codegened_functions) = aot::codegen_cgu_content(tcx, &mut module, cgu_name);
...
pub(crate) fn run_aot(
    ...
    let metadata_module =
        if need_metadata_module { Some(aot::emit_metadata_module(tcx, &metadata)) } else { None };
